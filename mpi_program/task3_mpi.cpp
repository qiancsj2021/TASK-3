...
void InitializeB(double* B, int M, int N, double h_x, double h_y, ProcInfo* Proc) {
	...
	for (i = 1; i <= Proc->size_x; i++) {
		for (j = 1; j <= Proc->size_y; j++) {
			...
			d_x = A1 + (double)idx_x * h_x;
			d_y = B1 + (double)idx_y * h_y;
...
//
void PrintVecToMat(double* A, int size_x, int size_y, ProcInfo* Proc) {
	int i, j;
	cout << "Rank = " << Proc->rank << "\n";
	for (i = 1; i <= size_x; i++) {
		for (j = 1; j <= size_y; j++) {
			cout << A[i * Proc->real_size_y + j] << " ";
		}
		cout << "\n";
	}
}
...
double Solve(int M, int N, double h_x, double h_y, MPI_Comm* Grid_Comm, ProcInfo* Proc, int* num_iter) {
	...
	string file_name = "./mpi_results/M" + to_string(M) + "_N" + to_string(N)
		+ "_procs_" + to_string(Proc->num_procs)
		+ "_block_" + to_string(Proc->coords[0]) + to_string(Proc->coords[1]) + ".txt";
	ofstream res_file(file_name, ios::out);
	res_file << Proc->size_x << "," << Proc->size_y << "\n";
	int idx_x, idx_y;
	...
	for (i = 1; i <= Proc->size_x; i++){
		for (j = 1; j <= Proc->size_y; j++){
			...
			res_file << u(idx_x, idx_y, h_x, h_y) << "," << w[i * Proc->real_size_y + j] << "\n";
		}
	}
	res_file.close();
...
int main(int argc, char* argv[]) {
	...
	// Check each process block coordinates, size, starting point & whether it gets the correct neighbor rank
	//cout << "rank " << Proc.rank << " | corrds_i, corrds_j " << Proc.coords[0] << " " << Proc.coords[1] << " | size_x, size_y = " << Proc.size_x << " " << Proc.size_y << " | i_beg, i_end = " << Proc.i_beg << " " << Proc.i_end << ", j_beg, j_end = " << Proc.j_beg << " " << Proc.j_end << "\nUp: " << Proc.nb_up << "; Down: " << Proc.nb_down << "; Left: " << Proc.nb_left << "; Right: " << Proc.nb_right << "\n\n";
